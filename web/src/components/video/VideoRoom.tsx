...
const BG = '#1a1a2e';
const BAR_BG = 'rgba(32,33,36,0.92)';
...
function buildRoomOptions(opts: {
  ...
  return {
    ...
    videoCaptureDefaults: {
      resolution: { width: 1280, height: 720, frameRate: 30, aspectRatio: 16 / 9 },
...
function PreJoin({
  ...
  // (Re)acquire preview stream when device/toggles change.
  useEffect(() => {
    let cancelled = false;
    stopStream();
    if (!camOn && !micOn) {
    ...
    navigator.mediaDevices
      ...
      .then(stream => {
        ...
        // Populate device lists (labels available once permission granted).
      ...
      .catch(() => {
        if (!cancelled) setPermError('Camera / microphone unavailable. You can still join.');
      });
  ...
  const join = () => {
    stopStream();
    onJoin({ camOn, micOn, videoDeviceId: videoDeviceId || undefined, audioDeviceId: audioDeviceId || undefined });
  };
...
function ControlButton({
  icon, label, onClick, danger, off, active, badge,
}: {
  ...
  return (
    <div style={{ position: 'relative', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 4 }}>
      {hover && (
        <div style={{ position: 'absolute', bottom: 'calc(100% + 8px)', whiteSpace: 'nowrap', padding: '5px 9px', borderRadius: 7, background: 'rgba(0,0,0,0.85)', color: '#fff', fontSize: 12, pointerEvents: 'none' }}>
          {label}
        </div>
      )}
      <button
  ...
  return (
    <div style={{
      ...
      <div style={{ height: 56, padding: '0 16px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', borderBottom: '1px solid rgba(255,255,255,0.08)' }}>
        ...
        <button onClick={onClose} aria-label="Close panel" style={{ width: 30, height: 30, borderRadius: 8, background: 'transparent', border: 'none', color: '#b6bbcd', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
          {Icons.close}
        </button>
      </div>
      {children}
...
function NotesPanel({ roomKey, onClose }: { roomKey: string; onClose: () => void }) {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      if (res.ok) {
        ...
        // Prefer whichever is non-empty; remote wins ties.
  ...
  // Final save on unmount (covers "leave call" / closing the panel).
  useEffect(() => () => {
    if (timer.current) clearTimeout(timer.current);
    ...
    <PanelShell title="Meeting Notes" onClose={onClose}>
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', padding: 14, minHeight: 0 }}>
        <textarea
          ...
          placeholder={loaded ? 'Type your meeting notes… (auto-saves)' : 'Loading…'}
          disabled={!loaded}
          style={{
            flex: 1, width: '100%', resize: 'none', borderRadius: 10, padding: 12,
            background: 'rgba(255,255,255,0.05)', border: '1px solid rgba(255,255,255,0.1)',
            color: '#e8ecff', fontSize: 14, lineHeight: 1.6, outline: 'none', fontFamily: 'inherit',
          }}
        />
...
function ChatPanel({ roomId, userId, onClose, onSeen }: { roomId: string; userId?: string; onClose: () => void; onSeen: () => void }) {
  ...
  const send = async () => {
    ...
    setSending(true);
    try { await sendRoomMessage(roomId, input.trim()); setInput(''); }
  ...
  return (
    <PanelShell title="In-call messages" onClose={onClose}>
      ...
      <div style={{ padding: 12, borderTop: '1px solid rgba(255,255,255,0.08)', display: 'flex', gap: 8 }}>
        ...
        />
        <button onClick={send} disabled={!input.trim() || sending} aria-label="Send" style={{ width: 40, height: 40, borderRadius: 10, background: input.trim() ? 'var(--brand-500,#2236B0)' : 'rgba(255,255,255,0.08)', color: '#fff', border: 'none', cursor: input.trim() ? 'pointer' : 'default', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
          {Icons.send}
        </button>
      </div>
    </PanelShell>
        ...
        {participants.map(p => {
          ...
          return (
            <div key={p.identity} style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '10px 10px', borderRadius: 10 }}>
              ...
              <span style={{ color: micMuted ? '#f87171' : '#8b91a8', display: 'flex' }}>
                <span style={{ width: 18, height: 18, display: 'flex' }}>{micMuted ? Icons.micOff : Icons.mic}</span>
              </span>
              <span style={{ color: camOff ? '#f87171' : '#8b91a8', display: 'flex' }}>
                <span style={{ width: 18, height: 18, display: 'flex' }}>{camOff ? Icons.camOff : Icons.cam}</span>
              </span>
            </div>
          );
        })}
...
function VideoRoomInner({ roomName, roomId, userId, onLeave, initialMic, initialCam }: {
  ...
  const toggleNoiseCancel = useCallback(async () => {
    ...
    if (audioTrack && 'restartTrack' in audioTrack) {
      try {
        await audioTrack.restartTrack({ noiseSuppression: next, echoCancellation: true, autoGainControl: true });
      } catch {
        // best effort — constraint may be unsupported
      }
    }
        ...
        <div style={{ flex: 1, minWidth: 0, display: 'flex', flexDirection: 'column', padding: 16, gap: 12, overflow: 'hidden' }}>
          {sharing ? (
            <div style={{ flex: 1, display: 'flex', gap: 12, minHeight: 0 }}>
              {/* Screen share — 75% */}
              <div style={{ flex: '0 0 75%', borderRadius: 12, overflow: 'hidden', background: '#000', minWidth: 0 }}>
            ...
            <div style={{
              ...
              maxWidth: count === 1 ? '80%' : '100%', margin: '0 auto', width: '100%',
            }}>
              {allParticipants.map(p => (
...
// Lightweight realtime watcher that bumps the unread badge when the chat
// panel is closed. Mounted only while the panel is not open.
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, zIndex: 1000, background: 'rgba(7,10,24,0.85)', backdropFilter: 'blur(8px)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 24 }} onClick={onClose}>
      <div onClick={e => e.stopPropagation()} style={{ width: 400, maxWidth: '100%', background: 'var(--bg-surface)', border: '1px solid var(--border-subtle)', borderRadius: 22, padding: 36, textAlign: 'center', boxShadow: 'var(--shadow-xl)' }}>
        ...
        <button onClick={onClose} style={{ height: 40, padding: '0 20px', borderRadius: 999, background: 'var(--bg-hover)', border: '1px solid var(--border-default)', fontSize: 14, fontWeight: 500 }}>
          Got it
        </button>
      </div>
    </div>
  ...
  if (!joined) {
    return (
      <PreJoin
        ...
        userName={userName}
        peerCount={peerCount}
        onJoin={s => { setSettings(s); setJoined(true); }}
        onCancel={onLeave}
      />
    );
  }
