  ...
  return (
    <>
      <nav style={{
        ...
        {/* Mobile hamburger */}
        <button className="md:hidden" onClick={() => setMobileMenu(true)} style={{ width: 36, height: 36, display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'var(--text-primary)' }}>
          ...
          <div className="stagger-in" style={{ display: 'flex', flexDirection: 'column', gap: 4, marginTop: 40 }}>
            {links.map((l) => (
              <button key={l.label} onClick={() => scrollTo(l.target)} style={{
                ...
                fontFamily: 'Fraunces, serif', fontWeight: 500, color: 'var(--text-primary)',
                borderBottom: '1px solid var(--border-subtle)',
              }}>{l.label}</button>
            ))}
            <Link href="/login" style={{ textAlign: 'left', padding: '16px 8px', fontSize: 24, fontFamily: 'Fraunces, serif', fontWeight: 500, color: 'var(--text-primary)', borderBottom: '1px solid var(--border-subtle)', display: 'block' }}>Log in</Link>
            <Link href="/signup" className="landing-cta" style={{ marginTop: 16, justifyContent: 'center', display: 'inline-flex' }}>Get Started Free</Link>
          </div>
        </div>
      )}
    </>
    ...
    <>
      {tiles.map((t, i) => (
        <div key={i} className="hidden md:block" style={{
          position: 'absolute', left: t.x, top: t.y, width: 220,
          background: 'var(--bg-surface)', border: '1px solid var(--border-subtle)',
          borderRadius: 16, padding: 14, boxShadow: 'var(--shadow-lg)',
...
function Hero() {
  return (
    <section style={{
      ...
      padding: '120px 24px 80px', background: 'var(--bg-base)', overflow: 'hidden',
    }}>
      <div aria-hidden style={{ position: 'absolute', inset: 0, background: 'radial-gradient(ellipse 1100px 600px at 50% 30%, rgba(27,43,142,0.14) 0%, transparent 60%), radial-gradient(ellipse 700px 400px at 90% 90%, rgba(245,158,11,0.08) 0%, transparent 60%)', pointerEvents: 'none' }}/>
      <div aria-hidden style={{ position: 'absolute', top: '50%', left: '50%', width: 600, height: 600, marginTop: -300, marginLeft: -300, background: 'conic-gradient(from 0deg, rgba(27,43,142,0.16), rgba(123,143,232,0.08), rgba(245,158,11,0.10), rgba(27,43,142,0.16))', filter: 'blur(80px)', animation: 'orbit 28s linear infinite', borderRadius: '50%', opacity: 0.6, pointerEvents: 'none' }}/>
      <FloatingMockups />

      <div style={{ position: 'relative', maxWidth: 780, textAlign: 'center', zIndex: 2 }} className="stagger-in">
        <div style={{ display: 'inline-flex', alignItems: 'center', gap: 6, padding: '6px 14px', borderRadius: 999, background: 'var(--glass-brand-bg)', border: '1px solid rgba(27,43,142,0.20)', color: 'var(--brand-600)', fontSize: 13, fontWeight: 500, marginBottom: 28 }}>
          <span style={{ fontSize: 14 }}>✦</span>
          <span>AI-Powered Study Environment</span>
        </div>

        <h1 style={{ fontFamily: 'Fraunces, serif', fontWeight: 800, fontSize: 'clamp(40px, 7vw, 80px)', lineHeight: 1.02, letterSpacing: '-0.025em', margin: 0, color: 'var(--text-primary)' }}>
          The study environment<br/>
          <span style={{ fontStyle: 'italic', fontWeight: 600, color: 'var(--brand-600)' }}>you were always missing.</span>
        </h1>

        <p style={{ fontFamily: 'Inter, system-ui', fontSize: 18, lineHeight: 1.55, color: 'var(--text-secondary)', maxWidth: 560, margin: '24px auto 0' }}>
        ...
        <div style={{ display: 'flex', gap: 14, justifyContent: 'center', flexWrap: 'wrap', marginTop: 36 }}>
          <Link href="/signup" className="landing-cta" style={{ height: 56, fontSize: 16, padding: '0 30px', display: 'inline-flex', alignItems: 'center' }}>
...
function ProblemSection() {
  const without = ['Studying alone, no accountability', 'Stuck on a doubt for hours', "No one to ask when you're lost", 'Motivation crashes by week 2'];
  const withElm = ['Live rooms — focus alongside others', 'Nova answers in seconds, anytime', 'Real mentors, 1:1, on demand', 'Streaks, analytics, momentum'];
  return (
    <section style={{ padding: '96px 24px', background: 'var(--bg-surface)', borderTop: '1px solid var(--border-subtle)', borderBottom: '1px solid var(--border-subtle)' }}>
      <div style={{ maxWidth: 1100, margin: '0 auto' }}>
        <div style={{ fontSize: 12, fontWeight: 700, letterSpacing: '0.12em', color: 'var(--brand-400)', textAlign: 'center', marginBottom: 14, textTransform: 'uppercase' }}>THE PROBLEM</div>
        <h2 style={{ fontFamily: 'Fraunces, serif', fontWeight: 700, fontSize: 'clamp(28px, 4.5vw, 40px)', lineHeight: 1.15, textAlign: 'center', maxWidth: 720, margin: '0 auto 56px', color: 'var(--text-primary)', letterSpacing: '-0.02em' }}>
          You have the content.<br/><span style={{ color: 'var(--text-secondary)' }}>Not the environment.</span>
        </h2>
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: 24 }}>
          <div style={{ background: 'var(--bg-subtle)', border: '1px solid var(--border-subtle)', borderLeft: '3px solid var(--danger-500)', borderRadius: 18, padding: 32 }}>
            ...
            <div style={{ fontFamily: 'Fraunces, serif', fontWeight: 600, fontSize: 22, marginBottom: 20, color: 'var(--text-primary)' }}>The grind alone.</div>
          ...
          <div style={{ background: 'var(--glass-brand-bg)', border: '1px solid rgba(27,43,142,0.20)', borderRadius: 18, padding: 32, position: 'relative', overflow: 'hidden' }}>
            <div aria-hidden style={{ position: 'absolute', top: -40, right: -40, width: 200, height: 200, borderRadius: '50%', background: 'radial-gradient(circle, rgba(27,43,142,0.15) 0%, transparent 70%)' }}/>
            ...
            <div style={{ fontFamily: 'Fraunces, serif', fontWeight: 600, fontSize: 22, marginBottom: 20, color: 'var(--text-primary)', position: 'relative' }}>The environment, finally.</div>
    ...
    <section id="features" style={{ padding: '96px 24px', background: 'var(--bg-base)' }}>
      <div style={{ maxWidth: 1180, margin: '0 auto' }}>
        <div style={{ textAlign: 'center', marginBottom: 56 }}>
          <div style={{ fontSize: 12, fontWeight: 700, letterSpacing: '0.12em', color: 'var(--brand-400)', marginBottom: 14, textTransform: 'uppercase' }}>WHAT'S INSIDE</div>
...
function FeatureCard({ icon, title, body, accent, special }: { icon: string; title: string; body: string; accent: string; special?: boolean }) {
  return (
    ...
    >
      ...
      <h3 style={{ fontFamily: 'Inter, system-ui', fontWeight: 600, fontSize: 19, color: 'var(--text-primary)', marginBottom: 8, letterSpacing: '-0.01em' }}>{title}</h3>
      <p style={{ fontFamily: 'Inter, system-ui', fontSize: 14.5, lineHeight: 1.55, color: 'var(--text-secondary)' }}>{body}</p>
    </div>
  );
}
    ...
    <section style={{ padding: '96px 24px', background: 'var(--bg-surface)', borderTop: '1px solid var(--border-subtle)', borderBottom: '1px solid var(--border-subtle)' }}>
      <div style={{ maxWidth: 1100, margin: '0 auto' }}>
        <div style={{ textAlign: 'center', marginBottom: 64 }}>
          <div style={{ fontSize: 12, fontWeight: 700, letterSpacing: '0.12em', color: 'var(--brand-400)', marginBottom: 14, textTransform: 'uppercase' }}>HOW IT WORKS</div>
          <h2 style={{ fontFamily: 'Fraunces, serif', fontWeight: 700, fontSize: 'clamp(28px, 4.5vw, 40px)', letterSpacing: '-0.02em', color: 'var(--text-primary)' }}>
        ...
        <div className="stagger-in" style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(260px, 1fr))', gap: 32, position: 'relative' }}>
          {steps.map((s, i) => (
            <div key={s.n} style={{ textAlign: 'center', position: 'relative' }}>
              <div style={{ width: 56, height: 56, borderRadius: '50%', background: 'var(--gradient-brand)', color: '#fff', display: 'inline-flex', alignItems: 'center', justifyContent: 'center', fontFamily: 'Fraunces, serif', fontWeight: 700, fontSize: 22, marginBottom: 20, boxShadow: '0 8px 22px rgba(27,43,142,0.30)', position: 'relative', zIndex: 2 }}>{s.n}</div>
              <h3 style={{ fontFamily: 'Inter, system-ui', fontWeight: 600, fontSize: 18, color: 'var(--text-primary)', marginBottom: 8, letterSpacing: '-0.01em' }}>{s.title}</h3>
              <p style={{ fontFamily: 'Inter, system-ui', fontSize: 14, lineHeight: 1.55, color: 'var(--text-secondary)', maxWidth: 280, margin: '0 auto' }}>{s.sub}</p>
              {i < steps.length - 1 && (
                <div className="hidden md:block" aria-hidden style={{ position: 'absolute', top: 28, left: 'calc(50% + 36px)', right: 'calc(-50% + 36px)', borderTop: '2px dashed var(--border-strong)', zIndex: 1 }}/>
              )}
            </div>
          ))}
        </div>
        ...
        <div className="marquee-track" style={{ animationPlayState: paused ? 'paused' : 'running' }}>
          {[...MENTORS, ...MENTORS].map((m, i) => (
            ...
            >
              ...
              <div style={{ fontSize: 12, color: 'var(--text-tertiary)', marginTop: 2 }}>{m.title}</div>
              <div style={{ marginTop: 8, display: 'flex', alignItems: 'center', gap: 4, fontSize: 12 }}>
                <span style={{ color: 'var(--gold-500)' }}>★</span>
                <span style={{ fontWeight: 600, color: 'var(--text-primary)' }}>{m.rating}</span>
              </div>
              <div style={{ marginTop: 10, display: 'flex', gap: 4, flexWrap: 'wrap' }}>
                {m.tags.map((t) => <span key={t} style={{ fontSize: 10, fontWeight: 600, padding: '3px 8px', borderRadius: 999, background: 'var(--brand-50)', color: 'var(--brand-600)' }}>{t}</span>)}
              </div>
              <div style={{ marginTop: 14, paddingTop: 12, borderTop: '1px solid var(--border-subtle)', fontSize: 12, color: 'var(--text-tertiary)' }}>
                from <strong style={{ color: 'var(--gold-600)', fontFamily: 'Inter', fontWeight: 700 }}>₹{m.price}</strong>
              </div>
            </div>
          ))}
        </div>
...
function Testimonials() {
  ...
  return (
    <section style={{ padding: '96px 24px', background: 'var(--bg-surface)', borderTop: '1px solid var(--border-subtle)', borderBottom: '1px solid var(--border-subtle)' }}>
      <div style={{ maxWidth: 1180, margin: '0 auto' }}>
        <div style={{ textAlign: 'center', marginBottom: 56 }}>
          <div style={{ fontSize: 12, fontWeight: 700, letterSpacing: '0.12em', color: 'var(--brand-400)', marginBottom: 14, textTransform: 'uppercase' }}>STUDENT VOICES</div>
          <h2 style={{ fontFamily: 'Fraunces, serif', fontWeight: 700, fontSize: 'clamp(28px, 4.5vw, 40px)', letterSpacing: '-0.02em', color: 'var(--text-primary)' }}>
            Built with them. <span style={{ fontStyle: 'italic', color: 'var(--brand-600)' }}>Loved by them.</span>
          </h2>
        </div>
        <div style={{ overflow: 'hidden' }}>
          <div style={{ display: 'flex', gap: 22, transform: `translateX(calc(-${idx * 100}% / 3))`, transition: 'transform 700ms var(--ease-out-expo)' }}>
            {TESTIMONIALS.map((t, i) => (
              <div key={i} style={{ flexShrink: 0, width: 'calc((100% - 44px) / 3)', minWidth: 280, background: 'var(--bg-base)', border: '1px solid var(--border-subtle)', borderRadius: 20, padding: 28 }}>
                <div style={{ fontFamily: 'Fraunces, serif', fontStyle: 'italic', fontWeight: 500, fontSize: 64, lineHeight: 0.6, color: 'var(--brand-300)', marginBottom: 16 }}>"</div>
                <p style={{ fontFamily: 'Inter, system-ui', fontSize: 15.5, lineHeight: 1.55, color: 'var(--text-primary)', marginBottom: 24 }}>{t.quote}</p>
                <div style={{ display: 'flex', alignItems: 'center', gap: 12, paddingTop: 16, borderTop: '1px solid var(--border-subtle)' }}>
                  <Avatar name={t.name} size={36} tint={t.tint} />
                  <div style={{ flex: 1 }}>
                    <div style={{ fontSize: 13, fontWeight: 600, color: 'var(--text-primary)' }}>{t.name}</div>
                    <div style={{ fontSize: 11, color: 'var(--text-tertiary)' }}>{t.field}</div>
                  </div>
        ...
        <div style={{ display: 'flex', gap: 6, justifyContent: 'center', marginTop: 32 }}>
          {TESTIMONIALS.map((_, i) => (
            <button key={i} onClick={() => setIdx(i)} style={{ width: i === idx ? 22 : 6, height: 6, borderRadius: 999, background: i === idx ? 'var(--brand-500)' : 'var(--border-strong)', transition: 'all 280ms var(--ease-smooth)' }}/>
          ))}
        </div>
      </div>
    </section>
...
function PricingPreview() {
  return (
    <section id="pricing" style={{ padding: '96px 24px', background: 'var(--bg-base)' }}>
      <div style={{ maxWidth: 1100, margin: '0 auto' }}>
        <div style={{ textAlign: 'center', marginBottom: 56 }}>
          <div style={{ fontSize: 12, fontWeight: 700, letterSpacing: '0.12em', color: 'var(--brand-400)', marginBottom: 14, textTransform: 'uppercase' }}>PRICING</div>
          <h2 style={{ fontFamily: 'Fraunces, serif', fontWeight: 700, fontSize: 'clamp(28px, 4.5vw, 40px)', letterSpacing: '-0.02em', color: 'var(--text-primary)' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(260px, 1fr))', gap: 22, alignItems: 'start' }}>
          {PLANS.map((p) => (
            <div key={p.name} className="price-card" style={{ transform: p.highlight ? 'scale(1.03)' : 'none', boxShadow: p.highlight ? 'var(--shadow-brand)' : 'var(--shadow-sm)', border: p.highlight ? '1.5px solid var(--brand-500)' : '1px solid var(--border-subtle)', background: p.highlight ? 'linear-gradient(180deg, var(--brand-50) 0%, var(--bg-surface) 30%)' : 'var(--bg-surface)' }}>
              {p.highlight && <div style={{ display: 'inline-flex', padding: '4px 12px', borderRadius: 999, background: 'var(--gradient-brand)', color: '#fff', fontSize: 11, fontWeight: 700, letterSpacing: '0.06em', textTransform: 'uppercase', marginBottom: 16 }}>Most popular</div>}
              <div style={{ fontFamily: 'Inter', fontWeight: 600, fontSize: 14, color: 'var(--text-secondary)', marginBottom: 8 }}>{p.name}</div>
              <div style={{ display: 'flex', alignItems: 'baseline', gap: 6, marginBottom: 4 }}>
                <span style={{ fontFamily: 'Fraunces, serif', fontWeight: 700, fontSize: 38, color: 'var(--text-primary)', letterSpacing: '-0.02em' }}>{p.price}</span>
              ...
              <ul style={{ listStyle: 'none', padding: 0, margin: '24px 0', display: 'flex', flexDirection: 'column', gap: 10 }}>
                {p.features.map((f) => <li key={f} style={{ display: 'flex', gap: 8, fontSize: 13.5, color: 'var(--text-secondary)' }}><span style={{ color: 'var(--mint-500)', fontWeight: 700 }}>✓</span>{f}</li>)}
              </ul>
              <Link href="/signup" style={{ width: '100%', height: 44, borderRadius: 999, background: p.highlight ? 'var(--gradient-brand)' : 'var(--bg-surface)', border: p.highlight ? 'none' : '1px solid var(--border-default)', color: p.highlight ? '#fff' : 'var(--text-primary)', fontFamily: 'Inter', fontWeight: 600, fontSize: 14, boxShadow: p.highlight ? '0 6px 18px rgba(27,43,142,0.30)' : 'none', transition: 'all 220ms', display: 'flex', alignItems: 'center', justifyContent: 'center', textDecoration: 'none' }}
        ...
        <div style={{ textAlign: 'center', marginTop: 40 }}>
          <Link href="/pricing" style={{ display: 'inline-flex', alignItems: 'center', gap: 6, color: 'var(--brand-500)', fontFamily: 'Inter', fontWeight: 600, fontSize: 14 }}
...
function FinalCTA() {
  return (
    <section id="community" style={{ padding: '120px 24px', textAlign: 'center', background: 'var(--bg-base)', position: 'relative', overflow: 'hidden' }}>
      <div aria-hidden style={{ position: 'absolute', inset: 0, background: 'radial-gradient(ellipse 800px 400px at 50% 50%, rgba(27,43,142,0.10) 0%, transparent 60%)' }}/>
      <div style={{ position: 'relative', maxWidth: 760, margin: '0 auto' }}>
        <h2 style={{ fontFamily: 'Fraunces, serif', fontWeight: 800, fontStyle: 'italic', fontSize: 'clamp(36px, 6vw, 64px)', lineHeight: 1.05, letterSpacing: '-0.025em', background: 'var(--gradient-brand)', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent', backgroundClip: 'text', marginBottom: 36 }}>
          Your best study year<br/>starts tonight.
        </h2>
        <Link href="/signup" className="landing-cta" style={{ height: 60, fontSize: 16, padding: '0 32px', display: 'inline-flex', alignItems: 'center' }}>
          Get Started Free <span style={{ marginLeft: 4 }}>→</span>
        </Link>
        <div style={{ marginTop: 32, display: 'flex', justifyContent: 'center', gap: 10, flexWrap: 'wrap' }}>
          {['Free plan forever', '7-day Pro trial', 'Cancel anytime'].map((t) => (
            <span key={t} style={{ display: 'inline-flex', alignItems: 'center', gap: 6, padding: '6px 14px', borderRadius: 999, background: 'var(--bg-surface)', border: '1px solid var(--border-subtle)', fontSize: 12, color: 'var(--text-secondary)', fontWeight: 500 }}>
      ...
      <div style={{ maxWidth: 1180, margin: '0 auto', display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(180px, 1fr))', gap: 40 }}>
        <div style={{ maxWidth: 280 }}>
          ...
          <div style={{ display: 'flex', gap: 8 }}>
            {['T', 'I', 'L'].map((s) => (
              <button key={s} style={{ width: 32, height: 32, borderRadius: 999, background: 'var(--bg-subtle)', color: 'var(--text-secondary)', display: 'inline-flex', alignItems: 'center', justifyContent: 'center', fontSize: 11, fontWeight: 600 }}>{s}</button>
            ))}
          </div>
        </div>
        {cols.map((c) => (
          <div key={c.title}>
            ...
            <ul style={{ listStyle: 'none', padding: 0, display: 'flex', flexDirection: 'column', gap: 10 }}>
              {c.items.map((item) => (
                <li key={item}>
                  <Link href={item === 'Privacy' ? '/legal/privacy' : item === 'Terms' ? '/legal/terms' : '#'} style={{ fontSize: 13, color: 'var(--text-tertiary)', cursor: 'pointer', textDecoration: 'none' }}>{item}</Link>
                </li>
              ))}
            </ul>
          </div>
