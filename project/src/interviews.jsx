...
const Interviews = ({ user, updateUser, navigate }) => {
  const safeUpdate = updateUser || (() => {});
  const safeNav = navigate || (() => {});
  const [view, setView] = useState('hub'); // hub | matching | live-coach | live-peer | ai-coach | feedback | coaches | company | schedule
  const [matchState, setMatchState] = useState(null);
  const [activeCompany, setActiveCompany] = useState(null);

  const startMatch = (mode) => {
    if (user && window.canQueuePeerInterview && !window.canQueuePeerInterview(user)) {
      window.openGate && window.openGate('peer-interview-quota', { plan: user.plan, quota: window.PEER_INTERVIEW_QUOTA[user.plan] });
      return;
    }
  ...
  const onMatched = () => {
    if (user) safeUpdate(prev => ({ ...prev, peerInterviewsToday: (prev.peerInterviewsToday || 0) + 1 }));
    setView('live-peer');
  };

  const joinScheduled = (u) => {
    setView('live-peer');
  };
  ...
  if (view === 'matching') {
    return <PeerMatching config={matchState} back={() => setView('hub')} onMatched={onMatched}/>;
  }
  if (view === 'coaches') {
    return <CoachListView coaches={INTERVIEW_COACHES} back={() => setView('hub')} onBook={(c) => { if (window.openBooking) window.openBooking(c); else setView('hub'); }}/>;
  }
  if (view === 'company' && activeCompany) {
    return <CompanyPrep company={activeCompany} back={() => { setActiveCompany(null); setView('hub'); }} onPracticePeer={() => startMatch({ type: 'coding', difficulty: 'mirror', duration: 30 })} onPracticeAI={() => setView('ai-coach')}/>;
  }
  if (view === 'schedule') {
    return <ScheduleAllView upcoming={UPCOMING} onJoin={joinScheduled} back={() => setView('hub')}/>;
  }
  if (view === 'live-peer') {
    ...
    <div className="page">
      ...
      <div style={{ marginBottom: 48 }}>
        <div style={{ display: 'flex', alignItems: 'baseline', justifyContent: 'space-between', marginBottom: 18 }}>
          ...
          <button onClick={() => setView('schedule')} className="btn btn-ghost btn-sm">View all</button>
        </div>
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
          {UPCOMING.map((u, i) => <UpcomingCard key={i} u={u} onJoin={() => joinScheduled(u)}/>)}
        </div>
      </div>
      ...
      <div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 12 }}>
          {[
            ...
            <div key={c.name} onClick={() => { setActiveCompany(c); setView('company'); }} className="card card-hover" style={{ padding: 22, cursor: 'pointer', animation: `fadeInUp 420ms ${i*60}ms var(--ease-out-expo) both` }}>
              <div style={{
...
const CoachPillar = () => {
  return (
    ...
    >
      ...
      <button onClick={() => setView('coaches')} className="btn btn-primary btn-md" style={{ width: '100%' }}>
...
const UpcomingCard = ({ u, onJoin }) => {
  ...
  return (
    <div className="card card-hover" style={{ padding: 20, display: 'flex', alignItems: 'center', gap: 14 }}>
      ...
      <button onClick={onJoin} className="btn btn-ghost btn-sm">Join</button>
    </div>
  );
};

// ─── Coach list view (all coaches) ───────────────
const CoachListView = ({ coaches, back, onBook }) => (
  <div className="page fade-in">
    ...
    <div className="label-sm" style={{ marginBottom: 10 }}>Coach me · 1:1</div>
    <h1 className="font-display" style={{ fontSize: 44, fontWeight: 500, letterSpacing: '-0.02em', marginBottom: 8 }}>
      Browse <span style={{ fontStyle: 'italic' }}>verified coaches</span>
    </h1>
    <p style={{ fontSize: 15, color: 'var(--text-secondary)', marginBottom: 32, maxWidth: 580 }}>
      All coaches go through a 3-round vetting process. Cancel free up to 24h before your session.
    </p>
    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: 14 }}>
      {coaches.map((c, i) => (
        <div key={c.id} className="card card-hover" style={{ padding: 22, display: 'flex', gap: 16, alignItems: 'flex-start' }}>
          <div style={{ position: 'relative', flexShrink: 0 }}>
            <Avatar name={c.name} size={56} tint={i}/>
            {c.online && <span style={{ position: 'absolute', bottom: 0, right: 0, width: 14, height: 14, borderRadius: 999, background: 'var(--mint-500)', border: '2px solid var(--bg-surface)' }}/>}
          </div>
          <div style={{ flex: 1, minWidth: 0 }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 2 }}>
              <span className="font-heading" style={{ fontSize: 15, fontWeight: 600 }}>{c.name}</span>
              <span style={{ width: 14, height: 14, borderRadius: 999, background: 'var(--brand-500)', color: '#fff', display: 'inline-flex', alignItems: 'center', justifyContent: 'center' }}><Icon name="check" size={8} stroke={3}/></span>
            </div>
            <div style={{ fontSize: 12, color: 'var(--text-tertiary)' }}>{c.title}</div>
            <div style={{ display: 'flex', gap: 6, marginTop: 10, flexWrap: 'wrap' }}>
              {c.specialties.map(s => <span key={s} className="chip chip-sm">{s}</span>)}
            </div>
            ...
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginTop: 14, paddingTop: 12, borderTop: '1px solid var(--border-subtle)' }}>
              <div>
                <div style={{ fontSize: 11, color: 'var(--mint-600)', fontFamily: 'Inter', fontWeight: 600 }}>{c.next}</div>
                <div style={{ fontSize: 13, color: 'var(--text-primary)', marginTop: 2, fontFamily: 'Inter', fontWeight: 600 }}>₹{c.price}/session</div>
              </div>
              <button onClick={() => onBook(c)} className="btn btn-primary btn-sm">Book session</button>
            </div>
          </div>
        </div>
      ))}
...
// ─── Schedule (View all) ────────────────────────
const ScheduleAllView = ({ upcoming, onJoin, back }) => {
  ...
  return (
    <div className="page fade-in">
      ...
      <h1 className="font-display" style={{ fontSize: 44, fontWeight: 500, letterSpacing: '-0.02em', marginBottom: 24 }}>
        Your <span style={{ fontStyle: 'italic' }}>schedule</span>
      </h1>
      <div className="toggle-row" style={{ marginBottom: 24 }}>
        {['upcoming','past'].map(f => <button key={f} onClick={() => setFilter(f)} className={filter === f ? 'active' : ''} style={{ textTransform: 'capitalize' }}>{f}</button>)}
      </div>
      <div style={{ display: 'grid', gridTemplateColumns: '1fr', gap: 12 }}>
        {(filter === 'upcoming' ? upcoming : []).map((u, i) => <UpcomingCard key={i} u={u} onJoin={() => onJoin(u)}/>)}
        {filter === 'past' && (
          <div className="card" style={{ padding: 40, textAlign: 'center', color: 'var(--text-tertiary)', fontSize: 14 }}>
            No past sessions yet. Your interview history will appear here.
          </div>
        )}
      </div>
    </div>
...
const COMPANY_QUESTIONS = {
  Google: [
    ...
    { q: 'Tell me about a time you disagreed with your manager. What happened?', t: 'Behavioral', d: 'Medium', freq: 62 },
...
const CompanyPrep = ({ company, back, onPracticePeer, onPracticeAI }) => {
  ...
  return (
    <div className="page fade-in">
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 18, marginBottom: 8 }}>
        <div style={{ width: 56, height: 56, borderRadius: 14, background: company.tone + '14', color: company.tone, display: 'flex', alignItems: 'center', justifyContent: 'center', fontFamily: 'Fraunces', fontSize: 26, fontWeight: 700 }}>{company.mono}</div>
        <div>
          <h1 className="font-display" style={{ fontSize: 40, fontWeight: 500, letterSpacing: '-0.02em', lineHeight: 1.05 }}>{company.name}</h1>
      ...
      <div style={{ margin: '24px 0', padding: 14, background: 'var(--bg-surface)', border: '1px solid var(--border-subtle)', borderRadius: 12, display: 'flex', alignItems: 'center', gap: 14 }}>
        <div style={{ flex: 1 }}>
          ...
          <div style={{ height: 6, background: 'var(--bg-subtle)', borderRadius: 999, overflow: 'hidden' }}>
            <div style={{ height: '100%', width: `${(doneCount / questions.length) * 100}%`, background: 'var(--gradient-brand)', borderRadius: 999, transition: 'width 360ms' }}/>
          </div>
        </div>
        <div style={{ fontFamily: 'JetBrains Mono', fontSize: 14, fontWeight: 600 }}>{doneCount}/{questions.length}</div>
      </div>
      <div className="toggle-row" style={{ marginBottom: 18 }}>
        {['All','Coding','System Design','Behavioral'].map(t => <button key={t} onClick={() => setTab(t)} className={tab === t ? 'active' : ''}>{t}</button>)}
      </div>
      <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
        {filtered.map((q, i) => (
          <div key={i} className="card card-hover" style={{ padding: 18, display: 'flex', alignItems: 'center', gap: 16 }}>
            <button onClick={() => setDone(d => ({ ...d, [q.q]: !d[q.q] }))} aria-label="Mark done" style={{ width: 22, height: 22, borderRadius: 6, border: '1.5px solid ' + (done[q.q] ? 'var(--mint-500)' : 'var(--border-default)'), background: done[q.q] ? 'var(--mint-500)' : 'transparent', color: '#fff', display: 'inline-flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
              {done[q.q] && <Icon name="check" size={12} stroke={3}/>}
            </button>
            <div style={{ flex: 1, minWidth: 0 }}>
              <div className="font-heading" style={{ fontSize: 14, fontWeight: 600, lineHeight: 1.4, textDecoration: done[q.q] ? 'line-through' : 'none', color: done[q.q] ? 'var(--text-tertiary)' : 'var(--text-primary)' }}>{q.q}</div>
              <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginTop: 6 }}>
                <span className="chip chip-sm">{q.t}</span>
                <span className="chip chip-sm" style={{ color: q.d === 'Hard' ? 'var(--danger-500)' : q.d === 'Medium' ? 'var(--amber-600)' : 'var(--mint-600)' }}>{q.d}</span>
                <span style={{ fontSize: 11, color: 'var(--text-tertiary)' }}>Asked in {q.freq} interviews this year</span>
              </div>
            </div>
            <div style={{ display: 'flex', gap: 6, flexShrink: 0 }}>
              <button onClick={onPracticeAI} className="btn btn-ghost btn-sm" title="Practice with AI Coach">Solo</button>
              <button onClick={onPracticePeer} className="btn btn-ghost btn-sm" title="Queue with peer">Peer</button>
            </div>
          </div>
        ))}
      </div>
