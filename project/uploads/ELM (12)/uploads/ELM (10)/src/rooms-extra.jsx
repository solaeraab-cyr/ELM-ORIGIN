...
const GATE_VARIANTS = {
  ...
  'collab-room': {
    icon: '🔗', headline: 'Collaborative rooms are a Pro feature',
    desc: 'Whiteboards, shared notes, and screen-share need a Pro plan. Public rooms stay free forever.',
    free: 'Free: Public rooms only',
    pro: 'Pro: Collaborative + Private + Group Interview',
  },
  'nova-limit': {
...
const GateModal = ({ variant = 'generic', onClose, onUpgrade }) => {
  const [shake, setShake] = useStateRX(false);
  const [exiting, setExiting] = useStateRX(false);
  const v = GATE_VARIANTS[variant] || GATE_VARIANTS.generic;
